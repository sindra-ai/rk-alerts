...
export async function listAccounts() {
  ...
  try {
    ...
    // keep only the whitelisted active accounts (by trailing digits of the id)
    const KEEP = ["4087", "4800"];
    const picked = all.filter((a) => KEEP.some((s) => String(a.id).endsWith(s)));
