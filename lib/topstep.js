...
export async function listAccounts() {
  ...
  try {
    ...
    // Show ONLY these specific accounts (match the account-number suffix in the name).
    // Add new funded account suffixes here when TopStep provisions them.
    const KEEP = ["80064800", "60374087", "88630208", "69658236", "41657198"];
    const hay = (a) => `${a.id} ${a.name || ""}`;
    const picked = all.filter((a) => KEEP.some((k) => hay(a).includes(k)));
