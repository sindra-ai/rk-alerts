...
export async function listAccounts() {
  ...
  try {
    ...
    // keep only tradeable (active) accounts; fall back to all if none qualify
    const tradeable = all.filter((a) => a.canTrade);
    const accounts = tradeable.length ? tradeable : all;
