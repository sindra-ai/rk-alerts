...
export async function listAccounts() {
  ...
  try {
    const data = await authed("/api/Account/search", { onlyActiveAccounts: false });
    // DIAGNOSTIC: log every account TopStep returns (name + id + simulated) so we can
    // see whether the funded EXPRESS accounts are in the response at all. Remove later.
    try {
      console.log("RAW_ACCOUNTS", JSON.stringify((data?.accounts || []).map(a => ({ id: a.id, name: a.name, balance: a.balance, simulated: a.simulated, canTrade: a.canTrade }))));
    } catch {}
    ...
    const KEEP = ["14215548", "85529904", "41657198", "69658236", "88630208"];
    ...
    // TEMP DEBUG: expose the full unfiltered list so we can see real account names/ids.
    const _debugAll = (data?.accounts || []).map((a) => ({ id: a.id, name: a.name, balance: a.balance, simulated: a.simulated, canTrade: a.canTrade }));
    return { configured: true, accounts, _debugAll };
  } catch (e) {
