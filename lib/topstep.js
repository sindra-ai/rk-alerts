...
export async function listAccounts() {
  ...
  try {
    ...
    // Show only YOUR accounts. Yours uniquely contain "DLL" in the name
    // (both Combines "50KTC-V2-DLL-..." and funded "EXPRESS-V2-DLL-...").
    // Old/other accounts lack "DLL", so this excludes them and auto-includes any
    // new funded DLL account the moment TopStep provisions it — no more manual edits.
    const picked = all.filter((a) => (a.name || "").toUpperCase().includes("-DLL-"));
